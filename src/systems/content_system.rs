...
pub fn parse_urgentcaredental_frontmatter(
    content: &str,
) -> Result<(UCDFrontmatter, String), Box<dyn std::error::Error>> {
    ...
    let end_marker = content[3..]
        .find("+++")
        .ok_or("No closing frontmatter marker")?;
            ...
            match key {
                ...
                "category" | "categories" => {
                    ...
                    if value.starts_with('[') && value.ends_with(']') {
                        ...
                        let single_cat =
                            normalize_category(value.trim_matches('"').trim_matches('\''));
...
pub fn get_all_posts() -> Vec<PostData<UCDFrontmatter>> {
    ...
    posts.retain(|post| {
        !post.frontmatter.exclude && !is_future_date(&post.frontmatter.date, &today)
    });
